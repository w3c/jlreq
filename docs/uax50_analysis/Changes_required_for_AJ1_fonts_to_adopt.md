...
### UAX#50の背景
印刷からデジタルテキストへのモデルの移行が UAX#50 の必要になる要因。印刷モデルでは作成者の画面上の結果を紙や PDF に固定できればよかったので、異なるフォントやアプリケーションで動作がバラバラでも困らなかった。
Web や EPUB などのデジタルテキストでは、受け取り手のアプリケーションやフォントに依存せず同じ結果を得られる必要がある。

## AJ1フォントがUAX#50に対応するために、必須の、及び必須でない変更点
### 縦書き字形をUAX#50の期待する方向に合わせるために必要な変更¹
vert を取り除く
...
JISで閉じていた時代と違い、複合フォントで構成される国際化システムにおいて、これらの文字を日本語フォントが持ち続ける意味がどのくらいあるのか、再考すべきかもしれない。
* ギリシア文字
* キリール文字
* 算術記号（ただし、ヒラギノは現在でもプロポーショナル）
* 矢印の一部など
JIS X 0213 の範囲内で該当する文字のリストは Appendix 1 を参照。

### 使われなくなるvert
これらの文字は、AJ1フォントが全角中心で回転されることを保証するために vert を持っている文字たち。事実上 R であり、UAX#50 と一致している。UAX#50 対応アプリケーションはこれらの文字をアプリケーションが回転させ、vert は使われない。
- ハイフンなどの欧文約物
- 矢印
- 大きな括弧の断片
- 罫線素片
...
4. UAX#50 では全角コロンに合わせて vert を加えて回転にしている。しかし、セミコロンには縦書きでの確立された用法がなく、中国語や韓国語のフォントでも正立。現在の AJ1 の動作が合理的に思われる
5. Dash であるので UAX#50 の回転が合理的だと思われる
...
対応に必須な変更が4文字なので、逆に言うと、 これら4文字の UAX#50 の定義を変更することで AJ1 フォントを as-is で UAX#50 対応とすることができる。つまり日本の主要なフォントが一挙に UAX#50 対応となる。

#### U+2016	双柱, DOUBLE VERTICAL LINE
- 現状のAJ1: R もしくは Tr 相当。vert が横書き字形を機械的に回転した字形を指している
- 現状のUAX#50: U
- → UAX#50 を R もしくは Tr に変更することで互換性を取ることができる
...
>文字の性質：絵文字的なのでUが本来ではある

#### U+FF1B	全角セミコロン, FULLWIDTH SEMICOLON
- 現状のAJ1: vert なし
- 現状のUAX#50: Tr で vert が回転を期待
- → UAX#50 を U もしくは Tu に変更することで互換性を取ることができる

>参考情報
>現在のアプリケーションの互換性：InDesign 正立。ブラウザはほぼ正立。例外は Firefox。フォントに関わらず回転させる
>石井さんによると、ドキュメントには明確に書かれていないが、Tr の場合 Tu の動作でも許容とのこと。その場合、UAX#50 ドキュメントの明確化で解決

#### U+3030	WAVY DASH
- 現状のAJ1: vert なし
- 現状のUAX#50: Tr で vert が回転を期待
- → UAX#50 を R にしてアプリケーションに回転させる、もしくは U に変更して正立させる、ことで互換性を取ることができる
