...
- 現状のUAX#50: U
- → UAX#50 を R もしくは Tr に変更することで互換性を取ることができる。vert がないフォントのあることを考えると、R がより広く互換性を確保できる

>参考情報
>現在のアプリケーションの互換性：InDesign は vert を使わず回転（R 動作）。ブラウザはブラウザとフォントの組み合わせで複雑にバラバラ
