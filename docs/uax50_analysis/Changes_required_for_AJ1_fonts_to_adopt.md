...
- 現状のAJ1: R もしくは Tr 相当。vert が横書き字形を機械的に回転した字形を指している
- 現状のUAX#50: U
- → UAX#50 を R に変更してアプリケーションが回転することで vert のないフォントに対しても互換性を取ることができる
- → もしくは、U+FF1A/B 全角コロンとセミコロンが Tr だが U/Tu 実装を例外的に許容しているのと同じ扱いにし、本来は U だが、Tu / Tr 動作を許容する
...
- → UAX#50 を R にしてアプリケーションに回転させる、もしくは U/Tu に変更して正立させる、ことで互換性を取ることができる。R がより広く実質的な互換性を確保できる
